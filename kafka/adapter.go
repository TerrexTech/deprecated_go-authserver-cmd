...
var peioStore = map[string]*ProducerESQueryIO{}
...
func (ka *Adapter) newProducerIO(id string, enableErrors bool) (*ProducerIO, error) {
	...
	pio := &ProducerIO{
		id:              id,
		inputChan:       (chan<- *model.KafkaResponse)(producerInputChan),
...
func (ka *Adapter) newProducerESQueryIO(topic string, id string, enableErrors bool) (*ProducerESQueryIO, error) {
	// Create Kafka Response-Producer
	resProducer, err := ka.producer(ka.Brokers)
	if err != nil {
		err = errors.Wrap(err, "Error Creating Response Producer")
	...
	// Setup Producer I/O channels
	producerInputChan := make(chan *model.EventStoreQuery)
	pio := &ProducerESQueryIO{
		id:              id,
		inputChan:       (chan<- *model.EventStoreQuery)(producerInputChan),
		producerErrChan: resProducer.Errors(),
	}
	...
	// The Kafka-Response post-processing the consumed events
	go func() {
		for esQuery := range producerInputChan {
			msgJSON, err := json.Marshal(esQuery)
			if err != nil {
				err = errors.Wrapf(err,
					"Error Marshalling EventStoreQuery with CorrelationID: %s and AggregateID: %d",
...
func (ka *Adapter) EnsureProducerESQueryIO(
	topic string,
	id string,
	enableErrors bool,
) (*ProducerESQueryIO, error) {
	if peioStore[id] == nil {
		p, err := ka.newProducerESQueryIO(topic, id, enableErrors)
		if err != nil {
			err = errors.Wrap(err, "Error creating ProducerESQueryIO")
			return nil, err
		}
		peioStore[id] = p
	}
